...
const AlertModal = ({visableAlertModal, message, res,onClose}) => {
  const [VisableAlertModal,setvisableAlertModal]=useState(visableAlertModal)
  const dispacth = useDispatch();
  const closeModal = () => {
    // dispacth(modifyAlertModal({show: false, message: null, res: null}));
    setvisableAlertModal(false)
    onClose()
  };
  useEffect(()=>{
    setvisableAlertModal(visableAlertModal)
  },[visableAlertModal])
    ...
    <Modal
      ...
      <View
        ...
        <View
          ...
          <Button
            mode="contained"
            buttonColor={res == 'succ' ? COLORS.primary : COLORS.primary}
            onPress={() => {
              closeModal();
            }}
            style={{
